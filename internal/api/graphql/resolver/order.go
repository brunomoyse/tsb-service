	...
	if input.IsOnlinePayment {
		...
		if user.NotifyOrderUpdates {
			go func() {
				...
				defer cancel()
				if err := es.SendOrderPendingEmail(*user, order.Language, *order, items); err != nil {
...
func (r *mutationResolver) RegisterDeviceToken(ctx context.Context, deviceToken string, platform string) (bool, error) {
	userID := utils.GetUserID(ctx)
...
func (r *mutationResolver) UnregisterDeviceToken(ctx context.Context, deviceToken string) (bool, error) {
	userID := utils.GetUserID(ctx)
	uid, err := uuid.Parse(userID)
	if err != nil {
		return false, fmt.Errorf("invalid user ID")
...
// IsManualAddress is the resolver for the isManualAddress field.
func (r *orderResolver) IsManualAddress(ctx context.Context, obj *model.Order) (bool, error) {
	if obj.IsManualAddr != nil {
		return *obj.IsManualAddr, nil
	}
	return false, nil
}
...
func (r *queryResolver) MyOrder(ctx context.Context, id uuid.UUID) (*model.Order, error) {
	...
	if o.UserID.String() != userID {
		return nil, &gqlerror.Error{
			Message:    "FORBIDDEN: order does not belong to caller",
			Path:       graphql.GetPath(ctx),
...
func (r *subscriptionResolver) MyOrderUpdated(ctx context.Context, orderID uuid.UUID) (<-chan *model.Order, error) {
	// The @auth directive guarantees a non-empty userID.
	userID := utils.GetUserID(ctx)
