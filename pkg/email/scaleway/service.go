...
// SendLoginOtpEmail sends a one-time passcode for passwordless login.
// The code is generated by Zitadel's Session API otpEmail challenge and
...
func SendLoginOtpEmail(user userDomain.User, lang string, code string) error {
	...
	// Determine the template path based on the user's language.
	path := fmt.Sprintf("templates/%s/login-otp", lang)

	htmlContent, err := renderLoginOtpEmailHTML(path, user, code)
	if err != nil {
		return fmt.Errorf("failed to render email template: %w", err)
	}

	plainTextContent, err := renderLoginOtpEmailText(path, user, code)
	if err != nil {
		return fmt.Errorf("failed to render email template: %w", err)
	}
	...
	logger.Debugf("Login OTP email sent to %s", user.Email)
